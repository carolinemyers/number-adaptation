...
//       // Finer intermediate ratios around the middle value (for better PSE extraction)
//       { left: 6, right: 9, middle: 8 },    // Close ratios (3:4)
//       { left: 8, right: 12, middle: 10 },   // Close ratios (2:3)
//       { left: 12, right: 18, middle: 15 },  // Close ratios (4:5)
  
//       // Ratios for broader Weber fraction coverage
//       { left: 5, right: 15, middle: 10 },   // Broader ratio (1:3)
//       { left: 10, right: 30, middle: 20 },  // Broader ratio (1:3)
//       { left: 5, right: 25, middle: 15 },   // Broader ratio (1:5)
//   ];

//   const adapterRatios = [
//     // Closer ratios for refined PSE estimation
//     { left: 6, right: 8, middle: 7 },     // 3:4 ratio
//     { left: 9, right: 12, middle: 10 },   // 3:4 ratio
//     { left: 12, right: 16, middle: 14 },  // 3:4 ratio

//     { left: 8, right: 12, middle: 10 },   // 2:3 ratio
//     { left: 10, right: 15, middle: 12 },  // 2:3 ratio
//     { left: 12, right: 18, middle: 15 },  // 2:3 ratio

//     // Original 1:2 configuration for larger adaptation effects (for comparison)
//     { left: 5, right: 20, middle: 10 },   // 1:2:4 ratio
//     { left: 7, right: 28, middle: 14 },   // 1:2 ratio
//     { left: 10, right: 40, middle: 20 }   // 1:2 ratio
// ];


//probe is unadapted, always 30 dots
//test varies and is adapted, can go all the way down to 12 dots. 

  
// For simplicity's sake, let's always assume right is probe/unadapted.

// const adapterRatios = [
//     // Closer ratios for refined PSE estimation
//     { leftAdapt: 400, rightAdapt: 30, leftTest: 7, rightTest: 30},     // 3:4 ratio
//     { left: 9, right: 12, middle: 10 },   // 3:4 ratio
//     { left: 12, right: 16, middle: 14 },  // 3:4 ratio
...
// 10 40 
// [10 comes back as 20, 16, 12] , 40

// { left: 7, right: 28, middle: 14 },   // 1:2 ratio

////ACTUAL PSE trials
    // Original 1:2 configuration for larger adaptation effects (for comparison)
    ///rightTest is always probe


   //adapting tolarge makes middle smaller

       // test always appears in same location as adapted stimulus and varies. 
    // probe always appears in other location, and is always the same amount. 

    //left adapter is always adapter, lefttest is always test
    // right adapter is always neutral, righttest is always probe

    //tests, to keep proportional to ours: 



//    const PSETrialValues_LargeAdapt = [
//     { leftAdapter: 80, rightAdapter: 40, rightTest: 40, leftTest: 50},   // 1:2:4 ratio
//     { leftAdapter: 80, rightAdapter: 40, rightTest: 40, leftTest: 60},
//     { leftAdapter: 80, rightAdapter: 40, rightTest: 40, leftTest: 70},
//     { leftAdapter: 80, rightAdapter: 40, rightTest: 40, leftTest: 80},
...
//     { rightAdapter: 80, leftAdapter: 40, leftTest: 40, rightTest: 50},   // 1:2:4 ratio
//     { rightAdapter: 80, leftAdapter: 40, leftTest: 40, rightTest: 60},
//     { rightAdapter: 80, leftAdapter: 40, leftTest: 40, rightTest: 70}, 
//     { rightAdapter: 80, leftAdapter: 40, leftTest: 40, rightTest: 80}, 
// ];


//     //adapting to small makes middle larger
//     const PSETrialValues_SmallAdapt = [
//         { leftAdapter: 40, rightAdapter: 80, rightTest: 80, leftTest: 40},  
//         { leftAdapter: 40, rightAdapter: 80, rightTest: 80, leftTest: 50},   // 1:2:4 ratio
//         { leftAdapter: 40, rightAdapter: 80, rightTest: 80, leftTest: 60},
//         { leftAdapter: 40, rightAdapter: 80, rightTest: 80, leftTest: 70},  

    
//         //reverse:
//           ///leftTest is always probe
//         { rightAdapter: 40, leftAdapter: 80, leftTest: 80, rightTest: 40}, 
//         { rightAdapter: 40, leftAdapter: 80, leftTest: 80, rightTest: 50},   // 1:2:4 ratio
//         { rightAdapter: 40, leftAdapter: 80, leftTest: 80, rightTest: 60},
//         { rightAdapter: 40, leftAdapter: 80, leftTest: 80, rightTest: 70}, 
    
//     ];

    //proportions of test from grasso et al:
    //0.25,0.29,0.35,0.42,0.50,0.60,0.71,0.83,1.00

    // so our values should be:
    //20,23,28,34,40,48,57,66,80

    //NOW VALUES SHOULD BE: 12, 14, 17, 20, 24, 29, 34, 40, 48
    ...
    const PSETrialValues_Baseline = [
       ...
       const PSETrialValues_SmallAdapt = [
        ...
        //     { leftAdapter: 24, rightAdapter: 48, rightTest: 48, leftTest: X},   // 1:2:4 ratio
        //     { leftAdapter: 24, rightAdapter: 48, rightTest: 48, leftTest: X},
        //     { leftAdapter: 24, rightAdapter: 48, rightTest: 48, leftTest: X},  
        ...
        //     //reverse:
        //       ///leftTest is always probe
        //     { rightAdapter: 24, leftAdapter: 48, leftTest: 48, rightTest: X}, 
        //     { rightAdapter: 24, leftAdapter: 48, leftTest: 48, rightTest: X},   // 1:2:4 ratio
        //     { rightAdapter: 24, leftAdapter: 48, leftTest: 48, rightTest: X},
        //     { rightAdapter: 24, leftAdapter: 48, leftTest: 48, rightTest: X}, 
...
// const PSETrialValues_LargeAdapt = [
//     { leftAdapter: 80, rightAdapter: 40, rightTest: 40, leftTest: 70},   // 1:2:4 ratio
//     { leftAdapter: 80, rightAdapter: 40, rightTest: 40, leftTest: 50},
//     { leftAdapter: 80, rightAdapter: 40, rightTest: 40, leftTest: 60},
//     { leftAdapter: 80, rightAdapter: 40, rightTest: 40, leftTest: 80},
...
//     //left adapter is always neutral, left test is always probe
//       ///leftTest is always probe
//     { rightAdapter: 80, leftAdapter: 40, leftTest: 40, rightTest: 70},   // 1:2:4 ratio
//     { rightAdapter: 80, leftAdapter: 40, leftTest: 40, rightTest: 60},
//     { rightAdapter: 80, leftAdapter: 40, leftTest: 40, rightTest: 50}, 
//     { rightAdapter: 80, leftAdapter: 40, leftTest: 40, rightTest: 80}, 
// ];
...
// const allImgs = ['img/helloWelcome.png','img/ladybug.png', 'img/TaskInstructions1.png','img/TaskInstructions1_cont.png','img/TaskInstructions2.png','img/TaskInstructions2_cont.png',
// 'img/TaskInstructions4.png','img/TaskInstructions4_cont.png','img/TaskInstructions5.png','img/TaskInstructions5_cont.png',
// 'img/TaskInstructions6.png','img/TaskInstructions6_cont.png',];
const allImgs = ['img/ladybug.png', 'Demo5_ReturnOrContinue.png'];
var preload = {
    type: 'preload',
    images: allImgs,
        ...
        on_load: function() {
          //  waitingForResponseFlag = false; 
            ...
            // ctx.fillText("Hello World", 10, 50);
                        ...
                        let center_y = canvas.height / 2;  // Common center for both
            ...
            function moveLadyBug(ladybug, boundaryX, boundaryY, minBoundaryX, side) {
                const speed = 0;//0; // Constant velocity
                let angle; 
                if (trial_count < 1) {
            ...
            function fadeDotsInAndOut(ladybug) {
                ...
                if (ladybug.alpha < 0.001 && trialType === 'adapt' && ladybug.fadeCount < ladybug.totalNumCycles && ladybug.fadeCount < 1) {
                //UNCOMMENTONLY IF we want to redraw dots in the same position within discs:
                //if (ladybug.alpha < 0.001 && trialType === 'adapt' && ladybug.fadeCount < ladybug.totalNumCycles && ladybug.fadeCount < 1) {   
...
var returnOrContinueTrial = {
    // use html-keyboard-response so we can fully control the HTML
    type: 'html-keyboard-response',
  
    // put both your image *and* the exact button‐group HTML here:
    stimulus: `
      <div style="position: relative; width:100vw; height:100vh; margin:0; overflow:hidden;">
        <img src="Demo5_ReturnOrContinue.png"
      ...
      <!-- your manual button group, centered over the image -->
      <div id="jspsych-html-button-response-btngroup"
           style="position:absolute;
                  top:90%;      /* tweak this % to move up/down */
                  left:50%;
                  transform:translate(-50%,-50%);
                  z-index:1000;
                  white-space:nowrap;">
        ...
        <div class="jspsych-html-button-response-button"
             style="display:inline-block; margin:0px 8px;"
             data-choice="1">
          <button class="jspsych-btn">Go to Dynamic Demo</button>
        </div>
        <div class="jspsych-html-button-response-button"
             style="display:inline-block; margin:0px 8px;"
             data-choice="2">
          <button class="jspsych-btn">Return to Main Page</button>
        </div>
      </div>
    `,
  
    // we won't use jsPsych's automatic choices
    choices: jsPsych.NO_KEYS,
    
    // listen for clicks on your manual buttons
    on_load: function() {
      document
        .querySelectorAll('#jspsych-html-button-response-btngroup .jspsych-btn')
        .forEach((btn, i) => {
          btn.addEventListener('click', () => {
            // finish the trial and hand back which button was clicked
            jsPsych.finishTrial({ response: i });
          });
        });
    },
  
    // handle the three responses exactly as before
    on_finish: function(data) {
      switch (data.response) {
        case 0:
          ...
          break;
        case 1:
          // go to other (dynamic) task
        //   const s = document.createElement('script');
        //   s.src = 'demo5_dynamic.js';
        //   document.body.appendChild(s);
        window.location.href = 'demo5_dynamic.html';
  ...
  timeline.push(returnOrContinueTrial);
